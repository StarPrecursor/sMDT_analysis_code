...
//  default text box locations  
  TPaveText *entryPT = new TPaveText(0.65,0.8,0.9,0.9,"NDC");
  TPaveText *t0PT = new TPaveText(0.65,0.7,0.9,0.8,"NDC");
  TPaveText *tmPT = new TPaveText(0.65,0.6,0.9,0.7,"NDC");
  TPaveText *tdPT = new TPaveText(0.65,0.5,0.9,0.6,"NDC");
...
//  Fit T0.  Used pardata for initial values if available, else estimate from histogram data
  ...
  ft0->SetParLimits(2,0.,1000.);  //Do not allow negative noise
  h->Fit("mt_t0_fermi","R");
...
//  }
  ...
  entryPT->AddText(Form("Total entries: %.0f", h->GetEntries()));
  entryPT->SetTextColor(kBlack);
  entryPT->Draw();
...
int T0Fit::AdcFit(TH1F *h, const int plot ) {
  ...
  TPaveText *adcPT = new TPaveText(0.58,0.315,0.98,0.635,"NDC");
